...
def main():
    ...
    logging.info(
        f"Results of random perceptron weigths multiplied by constant C = {C}:{results_c}"
    )
    logging.info(
        f"Are results the same for multiplied weights: {(results_c[0] == results[0])}"
    )
...
def build_perceptron_network(len_inputs, num_l0, constant_in):
    ...
    c_l0_outputs = get_network_level_output(
    ...
    c_l1_outputs = get_network_level_output(
...
def get_network_level_output(inputs, weights_level, biases_level):
    ...
    for i in range(num_neurons):
        perceptron_output_i = get_perceptron_output(
            inputs=inputs, weights=weights_level[i], bias=biases_level[i]
        )
